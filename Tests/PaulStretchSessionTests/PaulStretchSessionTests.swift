...
//
//  Effect stacks (order, bypass, duplicates, signature) and the session
//  bounce (loop phasing, lanes, pan law, mute/solo, determinism).
//
...
final class PaulStretchSessionTests: XCTestCase {
    ...
    func testDuplicateDevicesCascade() {
        ...
        let onceRMS = rms(once.l[0...])
        let twiceRMS = rms(twice.l[0...])
        XCTAssertLessThan(twiceRMS, onceRMS * 0.7,
    ...
    func testStackInterleavesAppleAndPureDevicesInOrder() {
        ...
        let outRMS = rms(out.l[0...])
        let inRMS = rms(input.l[0...])
        XCTAssertLessThan(outRMS, inRMS * 0.3, "two −6 dB EQ passes should land near −12 dB")
    }
    ...
    // MARK: - Placement (loop phasing, lanes, pan)

    func testLoopPhasingIsSampleExact() {
        var session = Session()
        session.durationSeconds = 1.0

        // Two looping DC voices with co-prime-ish lengths; every output
        // sample must be exactly the sum of the two loops read modulo.
        var a = Track(name: "A", source: .sample(SampleSource(audio: AudioReference(data: Data()))))
        var b = Track(name: "B", source: .sample(SampleSource(audio: AudioReference(data: Data()))))
        a.gain = 1; b.gain = 1
        let voiceA = ramp(frames: 1000)          // value = index / 1000
        let voiceB = ramp(frames: 707)
        b.loopPhaseSeconds = 100.0 / sr          // 100-frame phase offset
        session.tracks = [a, b]

        let out = try! SessionRenderer.render(session, voices: [a.id: voiceA, b.id: voiceB])
    ...
    func testStartOffsetAndOneShotEnd() {
        var session = Session()
        session.durationSeconds = 1.0
        var t = Track(name: "late", source: .sample(SampleSource(audio: AudioReference(data: Data()))))
        t.startSeconds = 0.5
        t.loops = false
        session.tracks = [t]

        let voice = dc(0.5, frames: 4410)        // 0.1 s one-shot
        let out = try! SessionRenderer.render(session, voices: [t.id: voice])

        let startFrame = Int(0.5 * sr)
        XCTAssertEqual(out.l[startFrame - 1], 0, "silent before the entry point")
        XCTAssertEqual(out.l[startFrame], 0.5, accuracy: 1e-6)
        XCTAssertEqual(out.l[startFrame + 4409], 0.5, accuracy: 1e-6)
        XCTAssertEqual(out.l[startFrame + 4410], 0, "one-shot must stop at its natural end")
    }
    ...
    func testMuteAndSolo() {
        ...
        var b = Track(name: "B", source: .sample(SampleSource(audio: AudioReference(data: Data()))))
        session.tracks = [a, b]
        let voices = [a.id: dc(0.25, frames: 100), b.id: dc(0.5, frames: 100)]
    ...
    func testPanLawUnityCentreAndCosineFarSide() {
        ...
        var t = Track(name: "left", source: .sample(SampleSource(audio: AudioReference(data: Data()))))
        t.pan = -1
        session.tracks = [t]

        let out = try! SessionRenderer.render(session, voices: [t.id: dc(0.5, frames: 500)])
    ...
    func testGainLaneShapesTheTrackOverSessionTime() {
        var session = Session()
        session.durationSeconds = 1.0
        var t = Track(name: "swell", source: .sample(SampleSource(audio: AudioReference(data: Data()))))
        t.gainLane = AutomationLane(points: [AutomationPoint(t: 0, v: 0),
                                             AutomationPoint(t: 1, v: 1)])
        session.tracks = [t]

        let frames = Int(sr * 1.0)
        let out = try! SessionRenderer.render(session, voices: [t.id: dc(0.5, frames: frames)])
    ...
    func testMasterStackShapesTheSum() {
        ...
        var t = Track(name: "A", source: .sample(SampleSource(audio: AudioReference(data: Data()))))
        session.tracks = [t]
        var pan = AutoPanSettings(); pan.depth = 1; pan.rateHz = 4
        session.master = EffectStack([EffectDevice(.autoPan(pan))])

        let voices = [t.id: tone(seconds: 0.5)]
    ...
    func testBounceIsDeterministic() throws {
        var session = Session()
        ...
        t.stack = EffectStack([EffectDevice(.shimmer(shimmer))])
        session.tracks = [t]
        ...
        // The voice cache key holds steady, and moves with the seed.
        let key = SessionRenderer.voiceCacheKey(for: t, sampleRate: sr)
        XCTAssertEqual(key, SessionRenderer.voiceCacheKey(for: t, sampleRate: sr))
        var reseeded = t
        if case .generative(var g) = reseeded.source { g.seed = 8; reseeded.source = .generative(g) }
        XCTAssertNotEqual(SessionRenderer.voiceCacheKey(for: reseeded, sampleRate: sr), key)
    }
    ...
    func testSessionCodableRoundTripAndTolerance() throws {
        ...
        var t = Track(name: "piano", source: .sample(SampleSource(audio: AudioReference(path: "/tmp/p.wav"))))
        t.loopPhaseSeconds = 13
        t.gainLane = AutomationLane(points: [AutomationPoint(t: 0, v: 1)])
        session.tracks = [t]
    ...
    // MARK: - Mixer (state logic; scheduling math is exercised via placement)
    ...
    func testMixerPrepareAndTransportState() {
        ...
        var a = Track(name: "A", source: .sample(SampleSource(audio: AudioReference(data: Data()))))
        session.tracks = [a]
        mixer.prepare(session: session, voices: [a.id: tone(seconds: 1)])
        ...
        mixer.setGain(trackID: a.id, 0.5)
        mixer.setSoloed(trackID: a.id, true)
        _ = a
    }
